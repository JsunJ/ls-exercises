...
// P
// input: a string
// output: the longest sentence found in the input based on number of words
//         the word count of the longest sentence
// rules: sentences end with periods, exclamation points, or question marks
//        a word is a sequence of characters that are not spaces or sentence-
...
function longestSentence(text) {
  let sentencesArray = text.match(/\w.*?[.!?]/g);

  let nonWordChars = /[\s.!?]/g;
  ...
  for (let i = 0; i < sentencesArray.length; i++) {
    ...
    for (let j = 0; j < currentSentence.length; j++) {
      let currentCharacter = currentSentence[j];
      if (nonWordChars.test(currentCharacter)) currentWordCount += 1;
    }
